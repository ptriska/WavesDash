...
app = dash.Dash(suppress_callback_exceptions = True, external_stylesheets=[dbc.themes.YETI, 'https://use.fontawesome.com/releases/v5.8.1/css/all.css'])
...
filter_col = dbc.Col([
								...
								dbc.Collapse([
									...
									dbc.InputGroup(
										[
											dcc.DatePickerRange(id = "date", display_format = "DD/MM/YYYY", style = {"minWidth":"100%", "marginBottom":"10px"})
...
"""
@app.callback(Output("variant","disabled"), Output("mutation","disabled"), Output("features","disabled"), Input("radioitems","value"))
...
"""
@app.callback(Output("needle_div","children"), Output("bigcol2","children"), Input("stackbar","clickData"), State("current_plant","data"),  State("plant_readable","data"))
...
"""
@app.callback(Output("map_col","width"), Output("pie_col","width"),Output("pie_div","style"),Output("toggle","children"), Input("toggle","n_clicks"), State("map_col","width"))
def toggle(i, w):
	if w == 6:
...
if __name__ == "__main__":
	app.run_server(debug=config.debug,dev_tools_ui=config.debug, dev_tools_props_check=config.debug, port = config.port, host='0.0.0.0')
